...
%This version refers to local angular parameters
%Replaced van Leer limiter with DCU
    ...
    for i=1:nx1
        ...
        for k=1:nxa(1)
            for l=1:num_phi_cells;   
                mu_coordinate(i,j,k,l,1) = mu(k,l,1);
                mu_coordinate(i,j,k,l,2) = mu(k,l,2)/rr2;
                mu_b_coordinate(i,j,k,l,1) = mu_b(k,l,1);
                mu_b_coordinate(i,j,k,l,2) = mu_b(k,l,2)/rr2;
                %add \hat{k}^3?
     ...
     for p=1:nx2_r
         for j=1:nxa(1)
             for l=1:num_phi_cells
                 ...
                 vtheta(n,p,j,l) = -sin(xAtemp)^3./rr(n) - 2*sin(xAtemp)*cos(xAtemp)^2; 
                 coordinate_source(n,p,j,l) = -(sin(xAtemp)^2*cos(xAtemp))./rr(n) + ...
                     4*sin(xAtemp)^2*cos(xAtemp) - 2*cos(xAtemp)^3; 
             end
         end
     end
 end
...
for i=0:nt
    ...
    i_flux = zeros(nx1,nx2,nxa(1),num_phi_cells,2);
    for j=1:nxa(1) 
        for l=1:num_phi_cells
            for n=num_ghost:nx1-1
                for p=num_ghost:nx2-1
                    %velocity at interface u_{j-1/2} via linear
                    %interpolation
                    ave_vel = 0.5*(mu_coordinate(n,p,j,l,1) + mu_coordinate(n-1,p,j,l,1));
                    if ave_vel > 0
                        i_flux(n,p,j,l,1) = ave_vel*intensity(n-1,p,j,l); %flux entering "left" boundary                          
                    elseif ave_vel < 0
                        i_flux(n,p,j,l,1) = ave_vel*intensity(n,p,j,l);  %flux leaving left boundary          
                    end
                end
            end 
            for n=is:ie
                for p=js:je
                    net_flux(n,p,j,l) = dt*C*(dxA1*dx2*rr_b(n+1-num_ghost,1).*i_flux(n+1,p,j,l,1) - dxA1*dx2*rr_b(n-num_ghost,1).*i_flux(n,p,j,l,1))./(cell_vol(n-num_ghost,p-num_ghost,j,l));
                end % all scaled by 31.5000
            end          
        end
    end
        ...
        for l=1:num_phi_cells
            for n=num_ghost:nx1-1
                for p=num_ghost:nx2-1
                    %velocity at interface u_{j-1/2} via linear
                    %interpolation
                    ave_vel = 0.5*(mu_coordinate(n,p,j,l,2) + mu_coordinate(n,p-1,j,l,2));
                    if ave_vel > 0
                        i_flux(n,p,j,l,2) = ave_vel*intensity(n,p-1,j,l); %flux entering "left" boundary                          
                    elseif ave_vel < 0
                        i_flux(n,p,j,l,2) = ave_vel*intensity(n,p,j,l);  %flux leaving left boundary          
                    end
                end
            end 
            for n=is:ie
                for p=js:je
                    net_flux(n,p,j,l) = net_flux(n,p,j,l) + dt*C*(dxA1*dx1*i_flux(n,p+1,j,l,2) - dxA1*dx1*i_flux(n,p,j,l,2))./(cell_vol(n-num_ghost,p-num_ghost,j,l));
                end %innermost ring scaled by 300
            end          
        end
    end
            ...
            for j=1:nxa(1) %edit out for easy periodic circular shifts
                for l=1:num_phi_cells
                    ...
                    %if vtheta(n,p,j,l) > 0 %counterclockwise speed
                        ...
                        if j-1 == 0
                          i_flux(n,p,j,l,1) = ave_vel*intensity(n+num_ghost,p+num_ghost,nxa(1),l); %flux entering "left" boundary                          
                        else
                          i_flux(n,p,j,l,1) = ave_vel*intensity(n+num_ghost,p+num_ghost,j-1,l); %flux entering "left" boundary                          
                        end
                    %elseif vtheta(n,p,j,l) < 0 %clockwise
                    elseif ave_vel < 0
                        i_flux(n,p,j,l,1) = ave_vel*intensity(n+num_ghost,p+num_ghost,j,l);  %flux leaving left boundary          
                    end
                end
